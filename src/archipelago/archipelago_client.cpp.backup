...
// Console output functions from the engine
...
void ArchipelagoClient::Authenticate(const std::string& slot, const std::string& password, int version) {
    ...
    packet.AddMember("name", rapidjson::Value(slot.c_str(), allocator), allocator);
    packet.AddMember("uuid", "", allocator);
    packet.AddMember("version", rapidjson::Value().SetObject(), allocator);
    packet["version"].AddMember("major", 0, allocator);
    packet["version"].AddMember("minor", 4, allocator);
    packet["version"].AddMember("build", 0, allocator);
    packet["version"].AddMember("class", "Version", allocator);
    
    packet.AddMember("items_handling", 0b111, allocator); // All items
    packet.AddMember("tags", rapidjson::Value().SetArray(), allocator);
    packet.AddMember("password", rapidjson::Value(password.c_str(), allocator), allocator);
